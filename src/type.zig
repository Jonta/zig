...
pub const Type = struct {
    ...
    pub fn abiSizeAdvanced(
        ...
        if (ty.ip_index != .none) switch (mod.intern_pool.indexToKey(ty.ip_index)) {
            ...
            .ptr_type => |ptr_type| switch (ptr_type.size) {
                .Slice => return .{ .scalar = @divExact(target.cpu.arch.ptrBitWidth(), 8) * 2 },
                else => return .{ .scalar = @divExact(target.cpu.arch.ptrBitWidth(), 8) },
            },
            .array_type => |array_type| {
    ...
    pub fn bitSizeAdvanced(
        ...
        if (ty.ip_index != .none) switch (mod.intern_pool.indexToKey(ty.ip_index)) {
            ...
            .ptr_type => |ptr_type| switch (ptr_type.size) {
                ...
                else => return target.cpu.arch.ptrBitWidth() * 2,
            },
            .array_type => |array_type| {
                const len = array_type.len + @boolToInt(array_type.sentinel != .none);
                if (len == 0) return 0;
    ...
    pub fn intInfo(starting_ty: Type, mod: *const Module) InternPool.Key.IntType {
        ...
        while (true) switch (ty.ip_index) {
            ...
            .usize_type => return .{ .signedness = .unsigned, .bits = target.cpu.arch.ptrBitWidth() },
            .isize_type => return .{ .signedness = .signed, .bits = target.cpu.arch.ptrBitWidth() },
    ...
    /// Asserts that the type is an integer.
    pub fn maxInt(ty: Type, mod: *Module) !Value {
        const info = ty.intInfo(mod);
